                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] xl:grid-cols-[1fr_450px] gap-6">
                    <div className="flex flex-col gap-6">
                        ...
                        <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm min-h-[300px]">
                            <h2 className="text-sm font-bold text-gray-900 mb-4 flex items-center justify-between"><span className="flex items-center gap-2"><ShoppingCart className="w-4 h-4 text-emerald-600" /> Itens no Carrinho</span>{cartItems.length > 0 && <span className="bg-blue-100 text-blue-700 font-bold text-xs px-2.5 py-0.5 rounded-full">{cartItems.length} {cartItems.length === 1 ? 'item' : 'itens'}</span>}</h2>
                            {cartItems.length === 0 ? <div className="text-center py-16 border-2 border-dashed border-gray-100 rounded-xl h-full flex flex-col items-center justify-center text-gray-400"><ScanBarcode className="w-10 h-10 mb-3 opacity-20" /><p className="text-sm font-semibold">Carrinho Vazio</p><p className="text-xs mt-1">Bipe os produtos ou busque manualmente.</p></div> : <div className="space-y-3 overflow-x-auto">{cartItems.map((item, idx) => (
                                <div key={item.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 bg-gray-50/50 hover:bg-white border hover:border-gray-200 rounded-xl transition-all"><div className="flex-1 min-w-0 pr-4"><div className="flex items-center gap-2"><span className="text-xs font-bold text-gray-400 w-5 shrink-0">{idx + 1}.</span><p className="text-sm font-bold text-gray-900 truncate">{item.name}</p></div>{(item.color || item.size) && <div className="flex flex-wrap items-center gap-1.5 ml-7 mt-0.5">{item.color && <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-gray-200 text-gray-700 uppercase">{item.color}</span>}{item.size && <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-gray-200 text-gray-700 uppercase">{item.size}</span>}</div>}<div className="flex items-center gap-3 ml-7 mt-1"><p className="text-[10px] text-gray-400 font-mono">{item.barcode || 'Sem código'}</p><span className="text-[10px] text-gray-300">|</span><p className="text-xs font-bold text-emerald-600">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(item.price)}</p></div></div><div className="flex items-center justify-between sm:justify-end gap-6 pt-2 sm:pt-0 border-t sm:border-t-0 border-gray-100"><div className="text-right w-32 shrink-0"><p className="text-[9px] text-gray-400 uppercase font-black">Total</p><p className="text-[15px] font-black text-slate-800">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(item.price * item.quantity)}</p></div><div className="flex items-center bg-white border border-gray-200 rounded-lg p-0.5"><button onClick={() => { const n = Math.max(0, item.quantity - 1); if (n === 0) setItemToRemove(item.id); else updateCartQuantity(item.id, String(n), item.unit); }} className="w-7 h-7 flex items-center justify-center text-gray-400 hover:text-red-500 rounded-md"><Minus className="w-3 h-3" /></button><input type="number" step={isIntegerUnit(item.unit) ? "1" : "0.01"} value={item.quantity} onChange={(e) => updateCartQuantity(item.id, e.target.value, item.unit)} onBlur={() => handleQuantityBlur(item.id)} className="w-10 h-7 text-center text-sm font-bold text-gray-900 border-none focus:ring-0 px-0" /><button onClick={() => updateCartQuantity(item.id, String(item.quantity + 1), item.unit)} className="w-7 h-7 flex items-center justify-center text-gray-400 hover:text-blue-600 rounded-md"><Plus className="w-3 h-3" /></button></div><button onClick={() => setItemToRemove(item.id)} className="w-10 h-10 flex items-center justify-center text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-colors"><Trash2 className="w-4 h-4" /></button></div></div>
                            ))}</div>}
                        </div>
                    </div>
