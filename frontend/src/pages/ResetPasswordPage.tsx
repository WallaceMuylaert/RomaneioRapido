...
export default function ResetPasswordPage() {
    ...
    return (
        <div className="min-h-screen relative flex items-center justify-center bg-text-primary overflow-hidden font-sans p-6 selection:bg-brand-500/30">
            <SEO
                title="Redefinir Senha | Romaneio Rápido"
                description="Defina uma nova senha para acessar o Romaneio Rápido."
                path="/reset-password"
                noindex
            />
            {/* Glowing Blobs */}
            ...
            <div className="w-full max-w-md relative z-10 bg-card rounded-[2rem] p-8 sm:p-10 shadow-2xl shadow-brand-900/50 animate-slide-up border border-card/10">
                <div className="text-center mb-10">
                    <div className="w-16 h-16 bg-gradient-to-tr from-brand-600 to-sky-500 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-xl shadow-primary/30 transform hover:-rotate-6 transition-all">
                ...
                {!isSuccess ? (
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="space-y-2">
                            <label className="text-[11px] font-black text-text-secondary uppercase tracking-widest ml-1">Nova Senha Invicto</label>
                            <div className="relative group/input">
                                <div className="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                                    <Lock className="h-4 w-4 text-text-secondary group-focus-within/input:text-brand-500 transition-colors" />
                                </div>
                                <input
                                    ...
                                    placeholder="••••••••••••"
                                    required
                                    className="w-full h-14 pl-12 pr-14 bg-background border-2 border-border rounded-2xl text-text-primary placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-brand-500/10 focus:border-brand-500 focus:bg-card transition-all font-bold text-sm shadow-sm"
                                />
                                <button
                                    type="button"
                                    onClick={() => setShowPassword(!showPassword)}
                                    className="absolute right-4 top-1/2 -translate-y-1/2 text-text-secondary hover:text-brand-600 p-2 transition-colors focus:outline-none"
                                >
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative group/input2">
                                <div className="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                                    <Lock className="h-4 w-4 text-text-secondary group-focus-within/input2:text-brand-500 transition-colors" />
                                </div>
                                <input
