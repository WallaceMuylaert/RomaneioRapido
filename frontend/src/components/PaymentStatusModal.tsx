...
export default function PaymentStatusModal({ status, isOpen, onClose, planName = 'Premium', error }: PaymentStatusModalProps) {
    ...
    return (
        ...
        >
            <div className={`fixed inset-0 bg-text-primary/50 backdrop-blur-md transition-opacity duration-700 ${animate ? 'opacity-100' : 'opacity-0'}`} />
            ...
            <div className="flex min-h-full items-center justify-center p-4 sm:p-6 sm:py-12 pointer-events-none">
                <div
                    ...
                    className={`relative w-full max-w-lg bg-card rounded-3xl sm:rounded-[2.5rem] border border-border overflow-hidden transition-all duration-700 transform pointer-events-auto ${animate ? 'scale-100 translate-y-0 opacity-100' : 'scale-90 translate-y-10 opacity-0'}`}
                >
                    ...
                    <div className={`p-8 sm:p-12 text-center relative transition-colors duration-500 ${isSuccess ? 'bg-gradient-to-br from-emerald-400 via-emerald-500 to-teal-600' :
                        ...
                        <div className="absolute top-0 left-0 w-full h-full overflow-hidden opacity-20 pointer-events-none">
                            <div className="absolute -top-10 -left-10 w-40 h-40 bg-card rounded-full blur-3xl" />
                            <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-card/30 rounded-full blur-3xl" />
                        </div>

                        {!isProcessing && (
                            <button
                                onClick={onClose}
                                className="absolute top-4 right-4 sm:top-6 sm:right-6 p-2 text-card/60 hover:text-card hover:bg-card/10 rounded-full transition-all"
                            >
                                <X className="w-5 h-5" />
                            </button>
                        )}
                        ...
                        <div className="relative inline-block">
                            <div className="w-20 h-20 sm:w-24 sm:h-24 bg-card/20 backdrop-blur-xl rounded-2xl sm:rounded-3xl flex items-center justify-center mb-4 sm:mb-6 mx-auto shadow-2xl">
                                {isProcessing && <Loader2 className="w-10 h-10 sm:w-12 sm:h-12 text-card animate-spin" />}
                                {isSuccess && <PartyPopper className="w-10 h-10 sm:w-12 sm:h-12 text-card animate-bounce-slow" />}
                                {isFailed && <AlertCircle className="w-10 h-10 sm:w-12 sm:h-12 text-card" />}
                                {isPending && <Clock className="w-10 h-10 sm:w-12 sm:h-12 text-card" />}
                            </div>
                            {isSuccess && <Sparkles className="absolute -top-2 -right-2 w-6 h-6 sm:w-8 sm:h-8 text-yellow-300 animate-pulse" />}
                        </div>

                        <h2 className="text-3xl sm:text-4xl font-black text-card tracking-tight mb-2">
                            {isProcessing ? 'Processando...' :
                                isSuccess ? 'Parabéns! 🎉' :
                                    isFailed ? 'Ops! Algo deu errado' :
                                        'Quase lá...'}
                        </h2>
                        <p className="text-card/90 font-bold text-base sm:text-lg max-w-xs mx-auto">
                    ...
                    <div className="p-8 sm:p-10 text-center">
                        <div className="text-text-secondary text-sm sm:text-base font-medium leading-relaxed mb-8 px-2">
                            {isProcessing ? (
                                ...
                                <p className="text-error font-bold">{error || 'Houve um problema ao processar seu pagamento. Por favor, tente novamente ou entre em contato com o suporte.'}</p>
                            ) : (
                        ...
                        {isSuccess && (
                            <div className="bg-emerald-50 rounded-2xl sm:rounded-[1.5rem] p-4 sm:p-6 mb-8 flex items-center gap-4 text-left border border-emerald-100">
                                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-card flex items-center justify-center shadow-sm shrink-0">
                        ...
                        {!isProcessing && (
                            <button
                                ...
                                className={`w-full h-12 sm:h-14 rounded-xl sm:rounded-[1.25rem] font-black text-base sm:text-lg transition-all shadow-xl active:scale-[0.98] ${isFailed ? 'bg-error hover:bg-error text-card shadow-red-100' :
                                        'bg-text-primary hover:bg-text-primary/90 text-card shadow-slate-200'
                                    }`}
                            >
                        ...
                        {isFailed && (
                            <a
                                ...
                                className="mt-4 flex items-center justify-center gap-2 text-sm font-bold text-text-secondary hover:text-brand-600 transition-colors"
                            >
                        ...
                        <p className="text-[10px] text-text-secondary mt-6 font-bold uppercase tracking-widest">Romaneio Rápido</p>
                    </div>
                </div>
            </div>
