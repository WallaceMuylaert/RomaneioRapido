...
export default function PlansGrid({ effectivePlanId, isSubscribing, handleSubscribe }: PlansGridProps) {
    ...
    return (
        ...
        >
            ...
            <button
                onClick={() => scroll('left')}
                className="absolute left-0 top-1/2 -translate-y-1/2 -ml-6 z-20 w-12 h-12 bg-white border border-slate-200 rounded-full items-center justify-center text-slate-400 hover:text-brand-600 shadow-xl transition-all opacity-0 group-hover/carousel:opacity-100 hidden xl:flex hover:scale-110 active:scale-95"
            >
            ...
            <button
                ...
                className="absolute right-0 top-1/2 -translate-y-1/2 -mr-6 z-20 w-12 h-12 bg-white border border-slate-200 rounded-full items-center justify-center text-slate-400 hover:text-brand-600 shadow-xl transition-all opacity-0 group-hover/carousel:opacity-100 hidden xl:flex hover:scale-110 active:scale-95"
            >
                    ...
                    return (
                        <div
                            ...
                            className={`flex-none w-[280px] lg:w-[320px] snap-start snap-always group p-6 lg:p-10 rounded-[2.5rem] border transition-all duration-700 flex flex-col h-full relative overflow-hidden ${isSelected
                                ...
                                : 'border-slate-200/80 bg-white hover:border-brand-300 hover:shadow-2xl hover:shadow-slate-200/50'
                        ...
                        >
                            ...
                            {isPopular && !isSelected && (
                                <div className="absolute top-0 right-0 p-3 lg:p-5">
                                    <div className="bg-brand-600 text-white text-[9px] font-black px-4 py-1.5 rounded-full uppercase tracking-tighter shadow-lg shadow-brand-500/30 animate-pulse">
                                        Destaque
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="flex flex-col items-start gap-4 mb-8">
                                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 transition-all duration-700 ${isSelected ? 'bg-brand-600 text-white shadow-lg shadow-brand-600/30' : 'bg-slate-50 text-slate-400 group-hover:bg-brand-100 group-hover:text-brand-600'
                                    }`}>
                                    <Zap className={`w-7 h-7 ${isSelected ? 'animate-pulse' : ''}`} />
                                </div>
                                <div className="space-y-1">
                                    <h4 className="font-extrabold text-slate-900 text-2xl tracking-tight leading-none">{p.name}</h4>
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{p.id === 'trial' ? 'Acesso inicial' : 'Assinatura mensal'}</p>
                                </div>
                            </div>
                            ...
                            <div className="mb-10 flex-grow">
                                <ul className="space-y-4">
                                    {p.features.map((feat, i) => (
                                        <li key={i} className="flex items-start gap-3.5 text-[14px] font-semibold text-slate-600 transition-colors group-hover:text-slate-900">
                            ...
                            <div className="border-t border-slate-100/80 pt-8 mt-auto flex flex-col items-center">
                                ...
                                <button
                                    ...
                                    className={`w-full h-14 rounded-2xl text-base font-black transition-all duration-300 flex items-center justify-center gap-2 active:scale-95 ${isSelected
                                        ...
                                        : 'bg-slate-900 text-white hover:bg-brand-600 shadow-xl shadow-slate-200 hover:shadow-brand-500/40'
                                        }`}
                                >
                                    {isSubscribing === p.id
            ...
            <div className="flex justify-center items-center gap-3 mt-4">
                {visiblePlans.map((_, i) => (
                    <button 
                        ...
                        className={`h-2 rounded-full transition-all duration-500 ${activeIndex === i ? 'w-8 bg-brand-500 shadow-lg shadow-brand-500/20' : 'w-2 bg-slate-200 hover:bg-slate-300'}`}
                    />
                ))}
            </div>
        </div>
